...
GROUPABLE_FIELDS = {
    ...
    "bit_rate": f"IIF(duration = 0, 0, file_size * {FORMATTED_DURATION_TIME_BASE} / duration)",
    "bit_depth": "bit_depth",
    ...
    "file_title_numeric": "pysaurus_text_with_numbers(pysaurus_get_file_title(filename), {padding})",
    "filename": "filename",
    "filename_numeric": "pysaurus_text_with_numbers(filename, {padding})",
    ...
    "title_numeric": "pysaurus_text_with_numbers(pysaurus_get_title(filename, meta_title), {padding})",
...
def get_sql_grouping(field: str, sorting: str, connection) -> str:
    from pysaurus.database.viewport.view_tools import GroupDef

    if sorting == GroupDef.LENGTH:
        assert field not in ("move_id", "size_length")

    sql_field = GROUPABLE_FIELDS[field]
    if field in ("file_title_numeric", "filename_numeric", "title_numeric"):
        ...
        sql_field = sql_field.format(padding=padding)
    return sql_field
