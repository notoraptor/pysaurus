...
from skullite.skullite import _SkullitPersistentConnection
...
def get_memory_sql_collection(name: str, *, home_dir=None):
    ...
    persistent_skullite = collection.db.__enter__().connect()
    assert isinstance(persistent_skullite, _SkullitPersistentConnection)

    persistent_skullite.connection.backup(memory_connection)
    persistent_skullite.connection = memory_connection
    return collection


def get_collection(name: str = "example_db_in_pysaurus"):
...
def test_edit_properties():
    ...
    with provider:
        ...
        old_values = collection.videos_tag_get("category", [84])[84]
        ...
        collection.videos_tag_set("category", {84: new_values})
        assert collection.videos_tag_get("category", [84])[84] == new_values
        provider.refresh()
        assert provider.get_view_indices() == [84, 21]

        collection.videos_tag_set("category", {84: old_values})
        assert collection.videos_tag_get("category", [84])[84] == old_values
        provider.refresh()
        assert provider.get_view_indices() == [21]
