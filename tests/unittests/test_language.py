...
def _install_french(tmp_path, translations: dict[str, str]):
    """Write english.txt + français.txt catalogs mapping given English texts."""
    english = {key_of(text): text for text in translations}
    french = {key_of(text): translated for text, translated in translations.items()}
    dff_dump(english, tmp_path / "english.txt")
    dff_dump(french, tmp_path / "français.txt")
...
def test_set_language_loads_translations(tmp_path):
    ...
    set_language("français", tmp_path)
    assert get_language() == "français"
...
def test_available_languages(tmp_path):
    _install_french(tmp_path, {"Move": "Déplacement"})
    dff_dump({"Key_1": "vieux texte"}, tmp_path / "français.obsolete.txt")
    assert available_languages(tmp_path) == ["english", "français"]


def test_available_languages_missing_folder(tmp_path):
    assert available_languages(tmp_path / "nope") == ["english"]
