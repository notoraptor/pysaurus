"""
Tests to compare JSON database and NewSQL database implementations.
...
class TestDatabaseComparison:
    """Compare JSON and NewSQL database implementations."""

    # =========================================================================
    # Basic info
    # =========================================================================

    def test_get_name(self, fake_old_database, fake_new_database):
        """Both databases should have the same name."""
        assert fake_old_database.get_name() == fake_new_database.get_name()

    def test_get_folders(self, fake_old_database, fake_new_database):
        """Both databases should return the same folders."""
        old_folders = sorted(str(f) for f in fake_old_database.get_folders())
        new_folders = sorted(str(f) for f in fake_new_database.get_folders())
    ...
    def test_get_videos_count(self, fake_old_database, fake_new_database):
        """Both databases should have the same number of videos."""
        old_videos = fake_old_database.get_videos()
        new_videos = fake_new_database.get_videos()
        assert len(old_videos) == len(new_videos)

    def test_get_videos_ids(self, fake_old_database, fake_new_database):
        """Both databases should have the same video IDs."""
        old_ids = sorted(v.video_id for v in fake_old_database.get_videos())
        new_ids = sorted(v.video_id for v in fake_new_database.get_videos())
        assert old_ids == new_ids

    def test_get_videos_filenames(self, fake_old_database, fake_new_database):
        """Both databases should have the same filenames."""
        old_filenames = sorted(str(v.filename) for v in fake_old_database.get_videos())
        new_filenames = sorted(str(v.filename) for v in fake_new_database.get_videos())
    ...
    def test_get_videos_where_video_id(self, fake_old_database, fake_new_database):
    ...
    def test_video_attributes_match(self, fake_old_database, fake_new_database):
        """All video attributes should match between databases."""
        old_videos = {v.video_id: v for v in fake_old_database.get_videos()}
        new_videos = {v.video_id: v for v in fake_new_database.get_videos()}
        ...
        for video_id in sample_ids:
            ...
            for attr in attributes:
                old_val = getattr(old_v, attr, None)
                new_val = getattr(new_v, attr, None)
    ...
    def test_count_videos_total(self, fake_old_database, fake_new_database):
        """Both should return the same total count."""
        assert fake_old_database.count_videos() == fake_new_database.count_videos()

    def test_count_videos_readable(self, fake_old_database, fake_new_database):
        """Both should return the same readable count."""
        assert fake_old_database.count_videos(
            "readable"
        ) == fake_new_database.count_videos("readable")

    def test_count_videos_with_thumbnails(self, fake_old_database, fake_new_database):
        """Both should return the same count with thumbnails."""
        assert fake_old_database.count_videos(
            "with_thumbnails"
        ) == fake_new_database.count_videos("with_thumbnails")

    # =========================================================================
    # has_video
    # =========================================================================

    def test_has_video_by_id(self, fake_old_database, fake_new_database):
        ...
        for video_id in sample_ids:
            assert fake_old_database.has_video(
                video_id=video_id
            ) == fake_new_database.has_video(video_id=video_id)

        # Test non-existent ID
        max_id = max(v.video_id for v in old_videos) if old_videos else 0
        assert fake_old_database.has_video(
            video_id=max_id + 9999
        ) == fake_new_database.has_video(video_id=max_id + 9999)

    def test_has_video_by_filename(self, fake_old_database, fake_new_database):
        ...
        for filename in sample_filenames:
            assert fake_old_database.has_video(
                filename=filename
            ) == fake_new_database.has_video(filename=filename)

    # =========================================================================
    # get_video_filename
    # =========================================================================

    def test_get_video_filename(self, fake_old_database, fake_new_database):
        ...
        for video_id in sample_ids:
            ...
            new_filename = fake_new_database.get_video_filename(video_id)
    ...
    def test_videos_get_terms_keys(self, fake_old_database, fake_new_database):
        """Both should return terms for the same video IDs."""
        old_terms = fake_old_database.videos_get_terms()
        new_terms = fake_new_database.videos_get_terms()

        assert sorted(old_terms.keys()) == sorted(new_terms.keys())

    def test_videos_get_terms_values(self, fake_old_database, fake_new_database):
    ...
    def test_videos_tag_get(self, fake_old_database, fake_new_database):
        ...
        for prop in old_props:
            ...
            new_tags = fake_new_database.videos_tag_get(name)
    ...
    def test_videos_tag_get_with_indices(self, fake_old_database, fake_new_database):
        ...
        for prop in old_props:
            ...
            new_tags = fake_new_database.videos_tag_get(name, indices=sample_ids)
    ...
    def test_videos_get_moves(self, fake_old_database, fake_new_database):
        """Both should return the same potential moves."""
        old_moves = list(fake_old_database.videos_get_moves())
        new_moves = list(fake_new_database.videos_get_moves())
    ...
    def test_get_unique_moves(self, fake_old_database, fake_new_database):
        """Both should return the same unique moves."""
        old_unique = fake_old_database.get_unique_moves()
        new_unique = fake_new_database.get_unique_moves()
    ...
    def test_thumbnail_presence(self, fake_old_database, fake_new_database):
        """Both should report the same thumbnail presence for each video."""
        old_videos = fake_old_database.get_videos()
        new_videos_map = {v.video_id: v for v in fake_new_database.get_videos()}
    ...
    def test_thumbnail_data(self, fake_old_database, fake_new_database):
...
class TestDatabaseComparisonEdgeCases:
    ...
    def test_empty_where_clause(self, fake_old_database, fake_new_database):
        """Empty where clause should return all videos."""
        old_all = fake_old_database.get_videos(where={})
        new_all = fake_new_database.get_videos(where={})
        assert len(old_all) == len(new_all)

    def test_multiple_where_conditions(self, fake_old_database, fake_new_database):
    ...
    def test_get_videos_list_of_ids(self, fake_old_database, fake_new_database):
        ...
        new_videos = fake_new_database.get_videos(where={"video_id": sample_ids})
