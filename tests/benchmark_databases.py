...
def run_benchmark(name: str, json_db, sql_db, json_fn, sql_fn, iterations: int = 5) -> BenchmarkResult:
...
def main():
    ...
    sample_filenames = [v.filename for v in json_db.get_videos(include=["filename"])][:100]
    ...
    results.append(run_benchmark(
        "sort(1000 videos, by title)",
        json_db, sql_db,
        lambda db: db.jsondb_provider_sort_video_indices(sample_1000, sorting_title),
        lambda db: db.jsondb_provider_sort_video_indices(sample_1000, sorting_title),
    ))
    ...
    results.append(run_benchmark(
        ...
        json_db, sql_db,
        lambda db: db.jsondb_provider_sort_video_indices(all_video_ids, sorting_title),
        lambda db: db.jsondb_provider_sort_video_indices(all_video_ids, sorting_title),
        iterations=3,
    ))
    ...
    results.append(run_benchmark(
        ...
        json_db, sql_db,
        lambda db: db.jsondb_provider_sort_video_indices(sample_1000, sorting_multi),
        lambda db: db.jsondb_provider_sort_video_indices(sample_1000, sorting_multi),
    ))
    ...
    sample_paths = [AbsolutePath(f) for f in [v.filename.path for v in json_db.get_videos()][:100]]
