...
class TestKeyboardEntry:
    ...
    def test_modifier_properties_rshift_lctrl_ralt_caps(self):
        ...
        mock_event = pygame.event.Event(pygame.KEYDOWN, {
            'mod': pygame.KMOD_RSHIFT | pygame.KMOD_LCTRL | pygame.KMOD_RALT | pygame.KMOD_CAPS,
            'key': pygame.K_b,
            'unicode': 'B'
        })
    ...
    def test_special_keys(self):
        ...
        various_prop_names = [
            prop_name for _, prop_name in navigation_keys
        ]

        for key_code, prop_name in navigation_keys:
            mock_event = pygame.event.Event(pygame.KEYUP, {
                'mod': 0,
                'key': key_code,
                'unicode': ''
            })
...
class TestMouseEventButtonProperties:
    ...
    def test_multiple_buttons_all_three(self):
        ...
        event = MouseEvent(buttons=[MouseButton.BUTTON_LEFT, MouseButton.BUTTON_MIDDLE, MouseButton.BUTTON_RIGHT])
        assert event.button_left is True
        assert event.button_middle is True
        assert event.button_right is True
