...
- Catalogs live in `pysaurus/languages/`, named by **ISO 639-1 code** (`en.txt` is the generated reference, `fr.txt` the French translation). Regenerate with `uv run poe i18n`: statically scans `say(...)` calls (lupdate-style), merges new/obsolete keys into translation files (orphaned translations move to `<code>.obsolete.txt`, never silently lost), and validates `{placeholders}`.
- Language identifiers are codes (`en`, `fr`), not full names — display names (endonyms shown in the UI) come from `pysaurus/languages/language_names.json`, generated from Qt's CLDR data by `uv run -m pysaurus.scripts.generate_language_names` (the only PySide6 touch in the language stack). Legacy full-name config values (`english`/`français`) are normalized on load via `language.canonical_language()`.
- The first argument of `say(...)` must be a string literal without trailing whitespace, and `say` must be imported unaliased (`from pysaurus.core.language import say`) — the extraction script fails otherwise; dynamic parts go through `**placeholders`. Switching to an unknown language code raises `UnknownLanguage` (`Application.set_language` validates against `available_languages()`).
