...
from pysaurus.utils.common import timestamp_microseconds, default
...
class Video(JSONCompatible):
    ...
    def __init__(self, properties=None, updated=False, duration_unit=duration.MICROSECONDS, **kwargs):
        ...
        self.movie_name = kwargs.get(strings.MOVIE_NAME, None)
        self.movie_title = kwargs.get(strings.MOVIE_TITLE, None)
        self.container_format = kwargs[strings.CONTAINER_FORMAT]
        self.size = kwargs[strings.SIZE]
        self.duration = duration.Duration(kwargs[strings.DURATION], duration_unit).to_microseconds()
        ...
        assert isinstance(self.sample_rate, (int, float, type(None))), self.sample_rate
