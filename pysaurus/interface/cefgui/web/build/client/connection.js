System.register(["./future.js", "./requestContext.js", "./exceptions.js"], function (_export, _context) {
  "use strict";

  var Future, RequestContext, Exceptions, Connection, ConnectionStatus;

  _export("Connection", void 0);

  return {
    setters: [function (_futureJs) {
      Future = _futureJs.Future;
    }, function (_requestContextJs) {
      RequestContext = _requestContextJs.RequestContext;
    }, function (_exceptionsJs) {
      Exceptions = _exceptionsJs.Exceptions;
    }],
    execute: function () {
      _export("ConnectionStatus", ConnectionStatus = {
      ...
      _export("Connection", Connection = class Connection {
        constructor(hostname, port, useSSL) {
          ...
          this.port = port; // Public properties.
          ...
          this.onNotification = null; // Private attributes.

          this.socket = null;
          this.status = ConnectionStatus.NOT_CONNECTED;
          this.waitingRequests = {};
          this.notificationManagers = {};
          this.futureConnection = new Future(); // Methods.
        ...
        reset() {
          ...
          if (!this.futureConnection.done()) this.futureConnection.setException(Exceptions.disconnected());
          this.waitingRequests = {};
          this.futureConnection = new Future(); // We don't reset notification managers, callbacks and address.
        }
        ...
        onOpen() {
          ...
          if (this.onError) this.socket.onerror = this.onError;else this.socket.onerror = null;
        ...
        manageResponse(response) {
          ...
          switch (response.type) {
            ...
            case 'data':
              if (!response.hasOwnProperty('data_type') || response.data_type !== requestContext.request.name) throw new Error('Invalid data type for received data response.');
              if (!response.hasOwnProperty('data')) throw new Error('No data field for received data response.');
              requestContext.future.setResult(response.data);
              break;

            case 'error':
              if (!response.hasOwnProperty('error_type')) throw new Error('Error response missing field error_type.');
              if (!response.hasOwnProperty('message')) throw new Error('Error response missing field message.');
        ...
        manageNotification(notification) {
          if (!notification.hasOwnProperty('name')) throw new Error('Missing notification name.');
          if (!notification.hasOwnProperty('parameters')) throw new Error('Missing notification parameters.');
        ...
        send(request) {
          if (this.status !== ConnectionStatus.CONNECTED) throw new Error('Socket not yet opened.');
          if (this.waitingRequests.hasOwnProperty(request.request_id)) throw new Error(`Request ID already used: ${request.request_id}`);
        ...
        setNotificationManager(name, callback) {
          if (callback) this.notificationManagers[name] = callback;else if (this.notificationManagers.hasOwnProperty(name)) delete this.notificationManagers[name];
        }
