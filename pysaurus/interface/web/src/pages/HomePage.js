...
class Monitoring extends React.Component {
    render() {
        ...
        const title = monitoring.title || this.context.text_done.format({count: current});
...
class NotificationRenderer extends React.Component {
    ...
    render() {
        ...
        if (this.hasOwnProperty(name)) {
            ...
            return <div key={i}><em>{this.context.text_notification_unknown}</em>: {message.message}</div>;
        }
    }
    ...
    DatabaseLoaded(app, message, i) {
        ...
        return (
            <div key={i}>
                <strong>{message.name === 'DatabaseSaved' ? this.context.text_database_saved : this.context.text_database_loaded}</strong>:
                {this.context.text_nb_entries.format({count: data.entries}) + ", "}
                {this.context.text_nb_discarded.format({count: data.discarded}) + ", "}
                {this.context.text_nb_unreadable_not_found.format({count: data.unreadable_not_found}) + ", "}
                {this.context.text_nb_unreadable_found.format({count: data.unreadable_found}) + ", "}
                {this.context.text_nb_readable_not_found.format({count: data.readable_not_found}) + ", "}
                {this.context.text_nb_readable_found_without_thumbnails.format({count: data.readable_found_without_thumbnails}) + ", "}
    ...
    DatabaseReady(app, message, i) {
        return <div key={i}><strong>{this.context.text_notification_database_ready}</strong></div>;
    }
    Done(app, message, i) {
        return <div key={i}><strong>{this.context.text_notification_done}</strong></div>;
    }
    Cancelled(app, message, i) {
        return <div key={i}><strong>{this.context.text_notification_cancelled}</strong></div>;
    }
    End(app, message, i) {
    ...
    FinishedCollectingVideos(app, message, i) {
        ...
        return (<div key={i}>{this.context.gui_home_collected_files.format({count}).markdown(true)}</div>);
    }
    MissingThumbnails(app, message, i) {
        ...
        if (names.length) {
            return (
                <div key={i}>
                    <div><strong>{this.context.text_notification_missing_thumbnails.format({count: names.length})}</strong>:</div>
                    {names.map((name, indexName) => <div key={indexName}><code>{name}</code></div>)}
                </div>
            );
        } else {
            return (<div key={i}><em>{this.context.text_notification_no_missing_thumbnails}</em></div>);
        }
    }
    ProfilingStart(app, message, i) {
        return (
            <div key={i}><span className="span-profiled">{this.context.text_profiling}</span> {message.notification.name}</div>
        );
    }
    ProfilingEnd(app, message, i) {
        return (
            <div key={i}>
                <span className="span-profiled">{message.notification.inplace ? `${this.context.text_profiling} / ` : ""}{this.context.text_profiled}</span>{" "}
    ...
    JobToDo(app, message, i) {
        ...
        if (title) {
            ...
            return (<div key={i}>{this.context.gui_home_to_load}: <strong>{total}{" "}{label}</strong></div>);
        } else {
            return (<div key={i}><em>{this.context.gui_home_to_load}: {this.context.text_nothing}</em></div>);
        }
    }
    NbMiniatures(app, message, i) {
        const total = message.notification.total;
        if (total) {
            return (<div key={i}><strong>{this.context.text_nb_miniatures_saved.format({count: total})}</strong></div>);
        } else {
            return (<div key={i}><em>{this.context.text_no_miniatures_saved}</em></div>);
        }
    }
...
export class HomePage extends React.Component {
    ...
    renderInitialButton() {
        if (this.props.parameters.onReady)
            ...
            return <button onClick={this.displayVideos}>{this.context.text_display_videos}</button>;
        else
            return <button disabled={true}>{ACTIONS[this.props.parameters.command[0]]} ...</button>;
    }

    renderMessages() {
        const output = this.state.messages.map((message, i) => <NotificationRenderer app={this} message={message} i={i}/>);
