...
export class FormVideosGrouping extends React.Component {
                ...
                <table className="from-videos-grouping first-td-text-right w-100">
                        ...
                        <td>
                            <select className="block"
                                ...
                                {this.state.isProperty ? (
                                    ...
                                    this.getFields().allowed.map((fieldOption, index) => (
                    ...
                    {this.state.isProperty || !this.getFields().fields[field].isOnlyMany() ? (
                        <tr>
                            ...
                            <td>
                                <label htmlFor="allow-singletons">{this.context.text_allow_singletons}</label>
                            </td>
                        </tr>
                    ) : (
                        <tr>
                            <td>&nbsp;</td>
                            <td><em>{this.context.text_singletons_auto_disabled}</em></td>
                        </tr>
                    )}
                    <tr>
                        <td className="label">
                            <label htmlFor="group-sorting">{this.context.text_sort_using}</label>
                        </td>
                        <td>
                            <select className="block"
                                    id="group-sorting"
                                ...
                                <option value="field">{this.context.text_field_value}</option>
                                {this.fieldIsString() ? <option value="length">{this.context.text_field_value_length}</option> : ""}
                                <option value="count">{this.context.text_group_size}</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <label htmlFor="group-reverse">{this.context.text_sort_reverse}</label>
                        </td>
                    </tr>
                    </tbody>
                </table>
    ...
    fieldIsString() {
        ...
        if (this.state.isProperty)
            return this.props.propertyMap[field].type === "str";
