        ...
        return (
            <div className="form-source-video">
                ...
                {this.state.paths.length ? (
                    <ul>{this.state.paths.map((path, index) => <li key={index}><strong>{path.replace(/-/g, '.')}</strong></li>)}</ul>
                ) : ''}
                <p className="submit"><button className="submit" onClick={this.submit}>select</button></p>
            </div>
        );
    }
                ...
                return (
                    <li key={index}>
                        ...
                        <p>
                            ...
                            <input type="checkbox" onChange={this.onChangeCheckBox} id={entryName + '0'} name={entryName} checked={hasPath}/>
                        </p>
                        )}
                    </li>
                );
