...
class VideoProvider:
    ...
    def get_view_file_size(self, view):
        return FileSize(sum(video.file_size for video in view))
    ...
    def count_groups(self):
        return (
            self.classifier_layer.count_groups()
            if self.grouping_layer.get_grouping()
            else 0
        )
