...
def job_videos_info(job):
    ...
    while cursor < count_tasks:
        ...
        results.extend(
            collector.collect(ListView(file_names, cursor, cursor + VIDEO_BATCH_SIZE))
        )
...
def job_videos_thumbnails(job):
    ...
    (
        ...
        job_id,
        notifier,
    ) = job  # type: (list, list, str, str, Notifier)
    cursor = 0
    count_tasks = len(file_names)
    ...
    while cursor < count_tasks:
        ...
        results.extend(
            generator.generate(
                ListView(file_names, cursor, cursor + VIDEO_THUMB_SIZE),
