...
from pysaurus.core.functions import is_sequence
...
class SortedSet:
    ...
    def __init__(self, element_type, content=(), already_sorted=True):
        ...
        if not is_sequence(content):
            raise TypeError(f'Expected a sequence, got {type(content)}')
        self.__type = element_type
        if already_sorted:
            self.__list = content if isinstance(content, list) else list(content)
        else:
            self.__list = []
    ...
    def get_next_value(self, element):
        """ Get lowest value in sorted set greater than given element, or None if such value
            does no exist in the sorted set. Given element may not exist in the sorted set.
        """
        assert isinstance(element, self.__type)
        if self.__list:
    ...
    def get_previous_value(self, element):
        """ Get greatest value in sorted set lesser than the given element, or None if such value
            does not exist in the sorted set. Given element may not exist in the sorted set.
        """
        assert isinstance(element, self.__type)
        if self.__list:
            best_position = bisect.bisect_left(self.__list, element)
