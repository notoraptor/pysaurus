...
def get_same_lengths(database: dict):
    ...
    if same_lengths:
        whole_info = '%d same length(S), %d video(s)' % (len(same_lengths), sum(len(elements) for elements in same_lengths.values()))
        ...
        html.write('<table cellspacing="0" style="width: 100%;">')
        for duration in sorted(same_lengths.keys()):
            elements = same_lengths[duration]  # type: list
            elements.sort(key=lambda v: v.filename)
            html.write('<tr><td colspan="7"><h2>%s, %d video(s)</h2></td></tr>' % (duration, len(elements)))
            for video in elements:  # type: Video
                html.write('<tr>')
                html.write('<td rowspan="2"><img src="file:///%s"/></td>' % video.thumbnail)
                html.write('<td class="link"><a target="_blank" href="file:///%s">%s</a></td>' % (urllib.parse.quote(video.filename.path), video.get_title()))
...
def find(database: dict, term: str):
    ...
    for video in found:
        print('\t"%s"\t%s' % (video.filename, Duration(video)))
