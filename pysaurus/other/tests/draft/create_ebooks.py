...
class Converter:
    ...
    def convert_to_ebook(self, directory, files, log_file):
        ...
        output_directory = AbsolutePath(
            upper_directory.path.replace(
                self.input_folder.path, self.output_folder.path
            )
        )
        ...
        if output_ebook.isfile() and output_ebook.get_size() > 1024 and not to_update:
            message(
                ...
                "EXISTS",
                directory,
                "%d image(s) => %s" % (len(files), output_ebook),
            )
        else:
            try:
                new_files = set()
                ...
                fatal_error(
                    log_file,
                    directory,
                    "%d image(s) => %s: %s" % (len(files), output_ebook, e),
                )
            else:
                message(
                    log_file,
                    ...
                    directory,
                    "%d image(s) => %s%s"
                    % (len(files), output_ebook, " (updated)" if to_update else ""),
                )
    ...
    def visit(self, directory: AbsolutePath, log_file):
        ...
        if folders and files:
            ...
            if any(extension in SUPPORTED_EXTENSIONS for extension in extensions):
                # Found images.
                for file in files:
                    if file.title != directory.title and REGEX_NUMBER.search(
                        file.title
                    ):
                        return fatal_error(
                            log_file,
                            directory,
                            "Found folders and an image with number in name: %s"
                            % file.get_basename(),
                        )
        ...
        if not folders and files:
            ...
            if extensions and all(
                extension in SUPPORTED_EXTENSIONS for extension in extensions
            ):
                ...
                return message(
                    ...
                    "PASSED",
                    directory,
                    "no supported extension: %s" % extensions,
                )
