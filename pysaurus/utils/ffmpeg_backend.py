...
def create_thumbnail(video: Video, output_folder: str, output_format: str = 'jpg'):
    output_file_path = os.path.abspath(os.path.join(output_folder, '%s.%s' % (video.title, output_format)))
    command = [
        'ffmpeg', '-y', '-ss', int(video.duration / 2), video.path, '-vframes 1', output_file_path]
    p = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    p.communicate()
    if not os.path.exists(output_file_path):
        raise FFmpegException(output_file_path)
