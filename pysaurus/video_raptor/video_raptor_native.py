...
class VideoRaptor(AbstractVideoRaptor):
    ...
    @classmethod
    def _job_video_to_json(cls, input_file_name, output_file_name, job_count, job_id):
    ...
    def collect_video_info(self, job: Job) -> list:
        ...
        self._job_video_to_json(
            list_file_path.path, json_file_path.path, len(job.batch), job.id
        )
