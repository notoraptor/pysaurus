...
from pysaurus.tests import test_images
from pysaurus.tests.test_images import SpacedPoints32To64
...
def dilate_miniature_data(data, width, height):
...
def simplify_images(miniatures: List[Miniature], group_min_size=1, pixel_distance_radius=1):
    ...
    group_computer = test_images.GroupComputer(group_min_size=group_min_size, similarity_percent=similarity_percent)
...
class Run:
    def __init__(self, files: List[str]):
        self.files = files
        self.api = API(TEST_LIST_FILE_PATH, update=False)
        self.videos = []
        for _, filename in files:
            video = self.api.database.get_video_from_filename(filename)
            assert video, filename
            self.videos.append(video)
        self.videos_dict = {v.filename.path: v for v in self.videos}  # type: Dict[str, Video]
        self.min_dict = {m.identifier: m
                    for m in self.api.database.ensure_miniatures(return_miniatures=True)
                    if m.identifier in self.videos_dict}
        self.miniatures = [self.min_dict[video.filename.path] for video in self.videos]
...
def main(
    ...
    min_dict = {m.identifier: m
                for m in api.database.ensure_miniatures(return_miniatures=True)
                if m.identifier in videos_dict}
    miniatures = [min_dict[video.filename.path] for video in videos]
    simplified_data, group_packs = simplify_images(
        miniatures, group_min_size=group_min_size, pixel_distance_radius=pixel_distance_radius
    )
    ...
    assert nb_pg == len(pg_to_bg), (nb_pg, len(pg_to_bg))
    print('Classifier', classifier)
    print('Nb. images', len(miniatures))
    print('Nb. pixel groups', nb_pg)
    print('Nb. basic groups', len(basic_groups))
    basic_groups = sorted(basic_groups)
    color_iterator = ColorIterator(2 ** 4)
    ...
    dec = 4
    surface = Draw(128 * len(miniatures) + dec * (len(miniatures) - 1), 128 * 3 + dec * 2)
    for i in range(len(miniatures)):
        m = miniatures[i]
        g = group_packs[i]
        d = [(0, 0, 0)] * m.width * m.height
