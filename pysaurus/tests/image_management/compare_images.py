...
from pysaurus.native.video_raptor import alignment as native_alignment
...
def similar_group_to_html_file(
    group_id, group, miniatures, database, html_dir, unique_id
):
    ...
    with StringPrinter() as html:
        ...
        html.write("<body>")
        html.write(
            "<h1>Group %s, %d files, min score %s, max score %s</h1>"
            % (group_id, size, min_score, max_score)
        )
        html.write("<table>")
        html.write("<thead>")
        html.write(
            '<tr><th class="header-images">Image</th><th class="header-details">Details</th></tr>'
        )
        html.write("<tbody>")
        for node in sorted(group):
            miniature_i = miniatures[node.node]
            thumb_path = database.get_video_from_filename(
                miniature_i.identifier
            ).thumbnail_path
            html.write("<tr>")
            ...
            html.write(
                '<div class="image-file"><strong><code>"%s"</code></strong></div>'
                % thumb_path
            )
            html.write(
                '<div class="origin-file"><em><code>"%s"</em></strong></div>'
                % miniature_i.identifier
            )
            html.write(
                "<div><code><strong><em>[%s]</em></strong></code></div>" % node.node
            )
            for output_node in sorted(node.edges):
                html.write(
                    '<div class="score"><code><em>With %s</em>: <strong>%s</strong></code></div>'
        ...
        output_file_name = AbsolutePath.join(
            html_dir, "sim.%s.%03d.html" % (unique_id, group_id)
        )
    ...
    while graph:
        ...
        while nodes_out:
            ...
            if v_out not in group:
                ...
                if v_out in graph:
                    nodes_out.extend(
                        (v_out, other_v, other_w)
                        for (other_v, other_w) in graph.pop(v_out)
                    )
...
def main():
    ...
    json_groups = [
        {
            miniatures[node.node]
            .identifier: database.get_video_from_filename(
                miniatures[node.node].identifier
            )
            .thumbnail_path.path
            for node in group
        }
