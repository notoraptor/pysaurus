...
class PixelMapper:
    ...
    def map_interval(pixel: Tuple[int, int, int], interval_length: int) -> Tuple:
        return tuple(
            int(v // interval_length) * interval_length
            for v in pixel
        )


class ProcessedImage:
    __slots__ = ('name', 'data', 'width', 'height', 'color_count', 'color_center', 'bg_to_pg')

    def __init__(self, name, data, width, height, bg_to_pg: Dict[BasicGroup, PixelGroup]):
        color_indices = {}
    ...
    def __str__(self):
        with StringPrinter() as printer:
            printer.write(self.name, len(self.color_center), f"color{'s' if len(self.color_center) > 1 else ''}")
            for i, color in enumerate(sorted(self.color_center)):
                printer.write(f"\t[{i + 1}] {color} count {self.color_count[color]} center {self.color_center[color]}")
            return str(printer)
...
class Space:
    ...
    def segment(
        ...
        for i in range(1, len(values)):
            if values[i] - values[i - 1] > limit:
                ...
                print(f'\t{axe_key} split {values[i - 1]} {values[i]} ({", ".join(img.name for img in cluster)}) vs ({", ".join(img.name for img in images if img not in cluster)})')
                cursor = i
        cluster = []
    ...
    def categorize(self):
        ...
        for prop in (self.C, self.X, self.Y):
            for color in self.colors:
                ...
                print('Segment', axe_key, 'before', len(segments), 'after', len(new_segments), 'total', sum(len(s) for s in new_segments))
    ...
    def categorize_by_group(self):
        ...
        for prop in (self.C, self.X, self.Y):
            for group in self.groups:
                ...
                new_segments = []
                for segment in segments:
                    clusters = self.segment_by_group(segment, axe_key, self.LIMITS[prop])
                    new_segments.extend(clusters)
                self.debug(new_segments)
                print('Segment', axe_key, 'before', len(segments), 'after', len(new_segments), 'total', sum(len(s) for s in new_segments))
                segments = new_segments
    ...
    def debug(self, segments: List[List[ProcessedImage]]):
        similarities = (("a1", "a2", "a3"), ("b1", "b2", "b3"), ("c1", "c2", "c3"), ("d1", "d2", "d3", "d4"))
        for i, segment in enumerate(segments):
            names = {image.name for image in segment}
            for similarity in similarities:
                if any(name in names for name in similarity) and not all(name in names for name in similarity):
                    missing = [name for name in similarity if name not in names]
                    print(f"Similarity [{i + 1}/{len(similarities)}] {similarity} is missing {missing}")
                    exit(-1)


class Run(TestAPI):
    __slots__ = ('mapped_files', 'nb_color_points', 'nb_position_points', 'nb_size_points', 'pixel_distance_radius', 'group_min_size')
    ...
    def run(self):
        ...
        group_computer = GroupComputer(group_min_size=self.group_min_size, similarity_percent=similarity_percent)
        img_to_pixel_groups = []
        img_to_basic_groups = []
        all_basic_groups = set()
        for m in self.miniatures:
            pixel_groups = group_computer.compute_groups(m)
            basic_groups = [pg.to_basic_group_intervals_alt(self.nb_color_points, self.nb_position_points, self.nb_size_points) for pg in pixel_groups]
        ...
        for i, miniature in enumerate(self.miniatures):
            output = [PixelMapper.map_interval(pixel, 256 // 8) for pixel in list(miniature.data())]
            mapped_miniatures.append(ProcessedImage(self.mapped_files[i][0], output, miniature.width, miniature.height, self.miniature_to_groups(group_computer, miniature)))
        space = Space(mapped_miniatures)
        space.categorize_by_group()
        dec = 4
        surface = Draw(128 * len(self.miniatures) + dec * (len(self.miniatures) - 1), 128 * 2 + dec)
